...
const rendeproductList = (productList) => {
    const dispatch = useDispatch();

    return Object.values(productList).map((product,i) => {
        return (
            <div className={`col-lg-3 col-md-6 special-grid  ${product.tags.join(" ")}`} key={i} >
                <div className="products-single fix">
                    <div className="box-img-hover">
                        {/* <div className="type-lb">
                            <p className="sale">Sale</p>
                        </div> */}
                        <img src={`/images/${product.featured_image}` }
                            className="img-fluid"
                            alt="Image"
                        />
...
export default function ShopListCategory() {
    const productList = useSelector((state) => {
        return useMemo(() => state.productList, [state.productList]);
    });
                        ...
                        <div className="product-categori">
                            ...
                            <div className="filter-sidebar-left">
                                ...
                                <div className="list-group list-group-collapse list-group-sm list-group-tree" id="list-group-men" data-children=".sub-men">
                                    {/* {productList.categorys.map((category, index) => (
                                        <a value={index} className="list-group-item list-group-item-action active">{category} <small className="text-muted">{productList.stock_qty}</small></a>
                                    ))} */}
